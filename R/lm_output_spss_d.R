...
#' lm_coef_spss_d(fit)
lm_coef_spss_d <- function (fit, sterne=T) {
  ...
  if (sterne) results <- cbind(results,data.frame(st=as.character(symnum(summary(fit)$coefficients[,4],corr=F,cutpoints = c(0, 0.001, 0.01, 0.05, 0.1,1),symbols = c("***","**","*","+"," ")))))
