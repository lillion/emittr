...
resultspre <- cbind(summary(fit)$coefficients,beta=coef(lmz),rbind(c(NA,NA,NA),cor_lm_d(fit,digits=stellen)))[c(1,2,5,3,4)]
results <- round.df(resultspre,stellen)
# if (fix) results <- fixp(results) ## neu
if (fix) results <- cbind(results[c(1,2,3,4)], pp <- round_ps(as.data.frame(resultspre)[[5]],stellen = stellen))
